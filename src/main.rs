...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Verify { address, key } => {
            ...
            if aes_key.is_none() {
                println!("[*] No account key provided — will try ECDH via Google Anti-Spoofing API");
            }
            println!();
            ...
            if let Some(ref info) = result.anti_spoofing_info {
                if let Some(ref name) = info.device_name {
                    println!("    Google device name   : {}", name);
                }
            }
            if let Some(ref strategy) = result.strategy_used {
                println!("    Winning strategy     : {}", strategy);
            }
            if let Some(ref diag) = result.hci_diagnostic {
                println!("    HCI write ack        : {}", diag.write_ack);
                if let Some((code, ref name)) = diag.att_error {
                    println!("    HCI ATT error        : 0x{:02X} — {}", code, name);
                }
                if !diag.raw_notifications.is_empty() {
                    for (i, n) in diag.raw_notifications.iter().enumerate() {
                        println!("    HCI raw notify[{}]    : {}", i, hex::encode(n));
                    }
                }
                if diag.disconnected {
                    println!("    HCI disconnect       : device dropped connection after probe");
                }
            }
            println!();
            ...
            if result.success {
                ...
                if let Some(ref provider_addr) = result.provider_address {
                    println!("    Provider BR/EDR addr : {}", provider_addr);
                }
                if let Some(ref raw) = result.raw_response {
                    println!("    Raw response (hex)   : {}", hex::encode(raw));
                }
            } else {
                ...
                println!("      • Device is patched (correct behaviour)");
                println!("      • Model ID not in Google Nearby database");
                println!("      • Device is out of range or connection dropped");
                println!("      • Try again with --key <account-key-hex> if you have previously paired");
            }
        }
