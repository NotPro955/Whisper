...
def transcribe_audio(audio_path: str | Path) -> str:
    """Transcribe a WAV file using the Python Whisper API when available, else the CLI."""
    ...
    if whisper_bin is None:
        raise RuntimeError(
            "No Whisper transcription backend is available. Install the Python whisper package or the whisper CLI."
        )

    result = subprocess.run(
        [whisper_bin, str(path), "--model", "base", "--output_format", "txt"],
        capture_output=True,
        text=True,
        check=False,
    )
    ...
    if result.returncode != 0:
        raise RuntimeError(result.stderr.strip() or "Whisper CLI transcription failed.")

    txt_path = path.with_suffix(".txt")
    if not txt_path.exists():
        raise RuntimeError(f"Whisper CLI did not generate a transcript at {txt_path}.")
